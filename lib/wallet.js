...
var BWS_INSTANCE_URL = 'https://dev-test.dash.org:3232/bws/api';
...
Wallet.prototype.joinWallet = function(opts, cb) {
    var self = this;

    this.client.joinWallet(opts.secret, opts.username, {}, function(err, wallet) {
        if (err) {
            return cb(err, null);
        }
        var wallet = JSON.parse(self.client.export());

        var result = {
            api_key: wallet.sharedEncryptingKey,
            username: opts.username,
            wallet: self.client.export()
        };
